...
describe('Query', () => {
  ...
  describe('Relation', () => {
    ...
    describe('One-to-Many (Owner: Foreign) Relationship', () => {
      it('Should be able to resolve relations on "find"', async () => {
        const sourceName = 'one_to_many_find_1'
        await setup(db.connection(), sourceName, (number) => ({
          key: `key-${number}`,
          value: `value-${number}`,
        }))

        const foreignName = 'one_to_many_find_2'
        await setup(db.connection(), foreignName, (number) => ({
          key: `key-${number}`,
          value: `value-${number}`,
          one_to_many_find_1_id: 1,
        }))

        const foreignSchema = base.set('table', foreignName)
        const schema = object({
          ...base.props,
          relation: foreignSchema.array(),
        }).set('table', sourceName)
      ...
      it('Should be able to insert (and insert relation)', async () => {
        const sourceName = 'one_to_many_insert_insert_1'
        await db.connection().from(sourceName).truncate()

        const foreignName = 'one_to_many_insert_insert_2'
        await db.connection().from(foreignName).truncate()
        ...
        expect(inserted[0]).toHaveProperty('value', 'value')
        expect(inserted[0]).toHaveProperty('relation')
        expect(inserted[0]!.relation).toHaveLength(1)
        expect(inserted[0]!.relation[0]).toHaveProperty('id')
        expect(inserted[0]!.relation[0]).toHaveProperty('key', 'key')
        expect(inserted[0]!.relation[0]).toHaveProperty('value', 'value')
        ...
        expect(result[0]).toHaveProperty('value', 'value')
        expect(result[0]).toHaveProperty('relation')
        expect(result[0]!.relation).toHaveLength(1)
        expect(result[0]!.relation[0]).toHaveProperty('id')
        expect(result[0]!.relation[0]).toHaveProperty('key', 'key')
        expect(result[0]!.relation[0]).toHaveProperty('value', 'value')
      })

      it('Should be able to insert (and save relation)', async () => {
        const sourceName = 'one_to_many_insert_save_1'
        await db.connection().from(sourceName).truncate()

        const foreignName = 'one_to_many_insert_save_2'
        await setup(db.connection(), foreignName, (number) => ({
          key: `key-${number}`,
          value: `value-${number}`,
        }))
        ...
        const schema = object({
          ...base.props,
          relation: foreignSchema.array().set('reference', 'strong'),
        }).set('table', sourceName)
      ...
      it('Should be able to save (and insert relation)', async () => {
        const sourceName = 'one_to_many_save_insert_1'
        await setup(db.connection(), sourceName, (number) => ({
          key: `key-${number}`,
          value: `value-${number}`,
        }))

        const foreignName = 'one_to_many_save_insert_2'
        await db.connection().from(foreignName).truncate()
        ...
        expect(saved[0]).toHaveProperty('value', 'value')
        expect(saved[0]).toHaveProperty('relation')
        expect(saved[0]!.relation).toHaveLength(1)
        expect(saved[0]!.relation[0]).toHaveProperty('id')
        expect(saved[0]!.relation[0]).toHaveProperty('key', 'key')
        expect(saved[0]!.relation[0]).toHaveProperty('value', 'value')
